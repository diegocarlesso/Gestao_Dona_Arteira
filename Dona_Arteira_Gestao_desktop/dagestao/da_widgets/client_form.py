...
class ClientFormDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.ed_notes.setFixedHeight(100)
    ...
    def _save(self):
        ...
        with SessionLocal() as s:
            ...
            c.phone = self.ed_phone.text().strip()
            c.street = self.ed_street.text().strip()
            c.number = self.ed_number.text().strip()
            c.neighborhood = self.ed_bairro.text().strip()
            c.city = self.ed_city.text().strip()
            c.state = self.cb_state.currentText()
            c.cpf_cnpj = self.ed_doc.text().strip()
            # >>> novos campos
            c.cep = self.ed_cep.text().strip()
            c.notes = self.ed_notes.toPlainText().strip()
            # <<<
            s.commit()
        self.accept()
