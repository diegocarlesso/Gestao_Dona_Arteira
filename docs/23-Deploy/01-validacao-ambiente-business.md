...
| **P-14** | **Cron exige o interpretador explícito** no comando | 🟡 Média | Só o caminho do `.php` faz o cron tentar executá-lo como programa → *Permission denied* enganoso (o arquivo não tem shebang). Some-se a isso que suexec/CageFS **recusa arquivos com `chmod 777`**. Regra: comando sempre como `/usr/bin/php /caminho/absoluto/arquivo.php`, arquivo em `644` |
...
| ~~P-6~~ | ~~`opcache` ausente~~ | — | ✅ **Falso negativo do meu script.** O `php -v` mostra *"with Zend OPcache v8.2.30"*: a extensão **está presente**. O teste usava `extension_loaded('opcache')`, mas ela se registra como **`Zend OPcache`**. Script corrigido com mapa de apelidos + `function_exists('opcache_get_status')` |
| **P-7** | **PHP 8.2.30 é o padrão** (`/usr/bin/php`) | 🟠 **Alta** *(elevada de Baixa)* | Não é preferência: **o suporte de segurança do PHP 8.2 termina em dezembro de 2026** — daqui a ~5 meses, com o projeto ainda no Gate 01/02. O PHP **8.4 tem suporte até dezembro de 2028** e está instalado (`/opt/alt/php84/usr/bin/php`; há também 8.1, 8.3 e 8.5). **Migrar para 8.4 em web e CLI antes do `composer create-project`** — depois, exige revalidar dependências |
