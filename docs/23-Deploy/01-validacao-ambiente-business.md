...
| P-4 | `symlink` bloqueada **no PHP** | 🟡 Média | ⚠️ **Conclusão anterior corrigida.** O bloqueio é da função `symlink()` do PHP (`disable_functions`); o comando **`ln -s` do shell é independente disso** e precisa ser testado separadamente ([23/02 §3.3](02-verificar-cron-e-docroot.md#arranjo-a-recomendado--aplicação-fora-alcançada-por-link-simbólico)). Se o shell permitir, **o deploy atômico por troca de link volta a ser possível** e o document root do subdomínio se resolve sem expor a aplicação. O que continua indisponível é o `artisan storage:link` — o link de storage é criado uma vez, à mão |
