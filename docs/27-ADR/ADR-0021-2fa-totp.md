...
> **Status:** ✅ **Aceito 2026-07-24** (dono) · **Data:** 2026-07-24 · **Decisores:** security-specialist, chief-architect — custo/dependência aprovados pelo dono
...
- TOTP via aplicativo autenticador (não SMS, não e-mail) — [docs/18-Usuarios/README.md §3.5](../18-Usuarios/README.md) e ADR-0005. **Por que não e-mail, agora que há SMTP:** a pergunta foi levantada pelo dono em 2026-07-24, quando o SMTP da Hostinger foi definido para o convite de usuário, e a resposta é que o canal disponível não é o problema — o problema é que **o mesmo e-mail já é o caminho de recuperação de senha** (reset por token de uso único, §3.5). Um OTP por e-mail colocaria os dois fatores no mesmo inbox: quem comprometesse a caixa postal obteria a senha (pelo reset) e o "segundo fator" (pelo OTP) pelo mesmo caminho, e o segundo fator deixaria de existir na prática. Isso derrubaria exatamente a proteção que BR-804 promete às contas de maior risco (`admin`/`finance`). Reconfirmado pelo dono na mesma data, inclusive como alternativa opcional — não entra em nenhum papel.
...
- `composer require laravel/fortify`. O `Laravel\Fortify\FortifyServiceProvider` **nunca é registrado** — nada do HTTP do pacote entra em execução; não existe `config/fortify.php` publicado, nenhuma `Features::` habilitada. Isso não é um hack: é o mesmo padrão que o próprio Laravel Jetstream usa oficialmente (consome as Actions do Fortify por baixo de uma UI própria, nunca as rotas do Fortify).
  - **Corrigido na implementação:** não basta omitir o provider de `bootstrap/providers.php`, como esta ADR dizia na versão Proposto. O Fortify declara o provider em `extra.laravel.providers` do próprio `composer.json`, então o **package discovery do Laravel o registra sozinho** — a omissão não barra nada. O bloqueio real é `extra.laravel.dont-discover` no nosso `composer.json`, e é assim que está feito. Um teste de arquitetura garante que o provider não esteja carregado, porque um `composer update` futuro não pode reintroduzi-lo em silêncio.
  - **Dependência não prevista:** o Fortify 1.37 exige `laravel/passkeys` (^0.2), que também se auto-registra e também entrou no `dont-discover`. Não muda a decisão — continua gratuito (MIT) e inerte —, mas aumenta o peso já reconhecido como dívida abaixo: são agora dois pacotes desligados no discovery para usar quatro classes.
- **Quem encripta as colunas: o Fortify, não os casts do model.** As Actions e o trait usam `Fortify::currentEncrypter()` internamente, e o `User` tinha `casts()` `encrypted`/`encrypted:array` nas mesmas três colunas — manter os dois encriptaria duas vezes (funcionaria por acidente, já que a leitura desfaz na ordem inversa, mas com um dono a mais do que o necessário). Os casts saem; a encriptação em repouso continua, com **um só dono**. `$hidden` e `$auditExclude` permanecem: são sobre exposição, não sobre encriptação.
